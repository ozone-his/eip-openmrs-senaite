...
public class CancelOrderToSenaiteRouteITest extends BaseCamelRoutesTest {
    ...
    public void setup() throws Exception {
    	...
    	advise("cancel-order-to-senaite", new AdviceWithRouteBuilder() {
    	    ...
    	    public void configure() {
    	    	weaveByToString(".*@@API/senaite/v1/search\\?getClientSampleID=\\$\\{exchangeProperty.order-to-cancel\\}\\&catalog=senaite_catalog_sample\\&complete=true\\&review_state=cancelled]").replace().to("mock:fetchcanceledOrderFromSenaiteEndpoint");
    	    	weaveByToString(".*@@API/senaite/v1/search\\?getClientSampleID=\\$\\{exchangeProperty.order-to-cancel\\}\\&catalog=senaite_catalog_sample\\&complete=true]").replace().to("mock:fetchActiveOrderFromSenaiteEndpoint");
    	    	weaveByToString(".*@@API/senaite/v1/update\\?throwExceptionOnFailure=false]").replace().to("mock:updateSenaiteWithoutThrowingEndpoint");
    ...
    public void shouldLogExistanceOfMultipleOrdersForCancelledOrder() throws Exception {
    	...
    	assertMessageLogged(Level.INFO, "Could not cancel order identified by: 'e585eced-0dd5-48eb-a267-a3049ab1ee538' due to multiple orders existing with same identifier on SENAITE");
    }
    
    private void setupExpectations() {
    	fetchcanceledOrderFromSenaiteEndpoint.whenAnyExchangeReceived(
			    exchange -> exchange.getIn().setBody("{\"count\":0,\"pagesize\":25,\"items\":[],\"page\":1,\"_runtime\":0.005473136901855469,\"next\":null,\"pages\":1,\"previous\":null}"));
    	...
    	updateSenaiteWithoutThrowingEndpoint.expectedBodiesReceived("{\"uid\": \"0b493f7934da4d618d57938e4fdc4246\", \"Client\": \"b774b05115824532bb589cbf47715570\", \"transition\": \"cancel\"}");
    }
}
