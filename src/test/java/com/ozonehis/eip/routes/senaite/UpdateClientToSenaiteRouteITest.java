...
@MockEndpoints
public class UpdateClientToSenaiteRouteITest extends BaseCamelRoutesTest {
    ...
    public void setup() throws Exception {
        ...
        advise("update-client-to-senaite", new AdviceWithRouteBuilder() {
            ...
            public void configure() {
                ...
                weaveByToString(
                                ".*/@@API/senaite/v1/search\\?portal_type=Client\\&getClientID=\\$\\{exchangeProperty.patient-id\\}]")
                        .replace()
                        .toD("mock:searchClientSenaiteEndpoint");
    ...
    public void shouldLogFailureToUpdatePatientDueToNoExistingPatientRecordInSenaite() throws Exception {
        ...
        assertMessageLogged(
                Level.INFO,
                "Could not update patient record identified by uuid = '77a7901-0a73-4849-8c36-fc5a6ae28503' due to no existing record with same identifier in SENAITE");
    }
    ...
    public void shouldLogFailureToUpdatePatientDueToMultipleExistingPatientRecordInSenaite() throws Exception {
        ...
        assertMessageLogged(
                Level.INFO,
                "Could not update patient record identified by uuid = '790177a-0a73-8c36-4849-fc285035a6ae' due to multiple existing records with same identifier in SENAITE");
    }

    private void setupExpectations() {
        searchClientSenaiteEndpoint.whenAnyExchangeReceived(exchange -> {
            if ("86f0b43e-12a2-4e98-9937-6c85d8f05d65".equals(exchange.getProperty("patient-id"))) {
                ...
                exchange.getIn()
                        .setBody(
                                "{\"count\":0,\"pagesize\":25,\"items\":[],\"page\":1,\"_runtime\":0.0023889541625976562,\"next\":null,\"pages\":1,\"previous\":null}");
            } else if ("790177a-0a73-8c36-4849-fc285035a6ae".equals(exchange.getProperty("patient-id"))) {
                exchange.getIn()
                        .setBody(
                                "{\"count\":3,\"pagesize\":25,\"items\":[\"Record1\",\"Record2\",\"Record3\"],\"page\":1,\"_runtime\":0.0023889541625976562,\"next\":null,\"pages\":1,\"previous\":null}");
            }
        });

        updateSenaiteEndpoint.expectedHeaderReceived(Exchange.HTTP_METHOD, "POST");
        updateSenaiteEndpoint.expectedBodiesReceived(
                "{\"title\":\"John Smitha Edited (HCD-3000006)\",\"uid\":\"06a29e6454464466876b718466176457\"}");
