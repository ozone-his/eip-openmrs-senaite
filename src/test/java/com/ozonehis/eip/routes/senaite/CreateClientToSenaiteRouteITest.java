...
public class CreateClientToSenaiteRouteITest extends BaseCamelRoutesTest {
    ...
    public void setup() throws Exception {
        loadXmlRoutesInDirectory("camel", "create-client-to-senaite-route.xml");

        advise("create-client-to-senaite", new AdviceWithRouteBuilder() {
            @Override
            public void configure() throws Exception {
                weaveByToString("To[direct:authenticate-to-senaite]").replace().toD("mock:authenticateToSenaiteRoute");
                weaveByToString(
                                ".*senaite/v1/search\\?portal_type=Client&getClientID=\\$\\{exchangeProperty.patient-id\\}]")
                        .replace()
                        .toD("mock:searchClientSenaiteEndpoint");
                weaveByToString(".*senaite/v1/create]").replace().toD("mock:createSenaiteEndpoint");
            }
        });
    ...
    private void setupExpectations() {
        ...
        createSenaiteEndpoint.expectedBodiesReceived(
                "{\"portal_type\":\"Client\",\"title\":\"John Smitha 2 (HCD-3000006)\",\"ClientID\":\"77a7901-0a73-4849-8c36-fc5a6ae28503\",\"parent_path\":\"/senaite/clients\"}");
        createSenaiteEndpoint.whenAnyExchangeReceived(
