...
public class PollSenaiteITest extends BaseCamelRoutesTest {
    ...
    public void setup() throws Exception {
        ...
        advise("poll-senaite", new AdviceWithRouteBuilder() {
            ...
            public void configure() {
                weaveByToString("To[direct:fetch-servicerequest-tasks-from-openmrs]")
                        .replace()
                        .toD("mock:fetchServiceRequestTasksRoute");
                weaveByToString(
                                ".*/ServiceRequest/\\$\\{exchangeProperty.service-request-id\\}\\?throwExceptionOnFailure=false]")
                        .replace()
                        .toD("mock:serviceRequestEndpoint");
                weaveByToString(".*/Task/\\$\\{exchangeProperty.task-id\\}]")
                        .replace()
                        .toD("mock:taskEndpoint");
                weaveByToString(
                                ".*/ws/rest/v1/encounter/\\$\\{exchangeProperty.service-request-encounter-reference\\}]")
                        .replace()
                        .toD("mock:encounterEndpoint");
                ...
                weaveByToString(
                                ".*@@API/senaite/v1/search\\?getClientSampleID=\\$\\{exchangeProperty.service-request-id\\}\\&getClientID=\\$\\{exchangeProperty.patient-id\\}\\&catalog=senaite_catalog_sample&complete=true]")
                        .replace()
                        .to("mock:analysisRequestSearchEndpoint");
                weaveByToString("To[direct:create-servicerequest-results-to-openmrs]")
                        .replace()
                        .to("mock:createServiceRequestResultsToOpenmrsRoute");
                weaveByToString("To[direct:update-servicerequest-task-to-openmrs]")
                        .replace()
                        .to("mock:updateServiceRequestTaskRoute");
            }
        });
    ...
    private void setupExpectations() {
        fetchServiceRequestTasksRoute.whenAnyExchangeReceived(
        ...
        serviceRequestEndpoint.whenAnyExchangeReceived(
                exchange -> exchange.getIn()
                        .setBody(
        ...
        encounterEndpoint.whenAnyExchangeReceived(
                exchange -> exchange.getIn()
                        .setBody(
        ...
        retrievePatientId.whenAnyExchangeReceived(
                exchange -> exchange.setProperty("patient-id", "some-unique-patient-id"));
        retrievePatientId.setResultWaitTime(resultWaitTimeMillis);

        analysisRequestSearchEndpoint.whenAnyExchangeReceived(
                exchange -> exchange.getIn()
                        .setBody(
