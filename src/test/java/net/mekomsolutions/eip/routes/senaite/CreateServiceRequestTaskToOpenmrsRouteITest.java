...
public class CreateServiceRequestTaskToOpenmrsRouteITest extends BaseWatcherRouteTest {  
    ...
    public void setup() throws Exception {
    	loadXmlRoutesInDirectory("senaite", "create-serviceRequestTask-toOpenmrs-route.xml");
    	RouteDefinition routeDefinition = camelContext.adapt(ModelCamelContext.class).getRouteDefinitions().stream().filter(routeDef -> "create-serviceRequestTask-toOpenmrs".equals(routeDef.getRouteId())).collect(Collectors.toList()).get(0);
    ...
    public void shouldCreateServiceRequestTaskOnOpenmrs() throws Exception {
    	...
    	// replay
    	producerTemplate.send("direct:create-serviceRequestTask-toOpenmrs", exchange);
